    ...
    const fetch_ = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/service-rates?filters[medical_service][slug][$eq]=${medicalServiceSlug}&populate[doctor][fields][0]=doctorName&populate[doctor][fields][1]=slug&populate[doctor][populate][image][fields][0]=url&populate[doctor][populate][bannerImage][fields][0]=url&populate[doctor][populate][clinic][fields][0]=title&populate[doctor][populate][clinic][fields][1]=slug&populate[doctor][populate][services][fields][0]=serviceName&populate[doctor][populate][services][fields][1]=slug`
        );
