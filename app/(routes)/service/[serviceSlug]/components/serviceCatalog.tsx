...
const ServiceCatalog = () => {
  ...
  useEffect(() => {
    if (!loading && Array.isArray(result)) {
      for (const doctor of result as DoctorType[]) {
        const matchedService = doctor.services?.find((s) => s.slug === categorySlug);
        if (matchedService) {
          setServiceTitle(matchedService.serviceName);
          return;
        }
      }
    }
  ...
  return (
    <div className="max-w-6xl py-4 mx-auto sm:py-16 sm:px-24">
      <h1 className="text-3xl font-medium mb-4">{serviceTitle}</h1>
      <Separator />
